...
export default function AsideFilter({ queryConfig, categories }: Props) {
  const { category: category_id } = queryConfig
  const {
    control,
    handleSubmit,
    trigger,
    ...
    <div className='py-4'>
          ...
          categories.map((category) => (
            <li className='py-2 pl-2' key={category._id}>
              <Link
                ...
                className={classNames('relative font-medium', { 'text-orange': category._id === category_id })}
              >
      ...
      <div className='my-5'>
        <form className='mt-2' onSubmit={onSubmit}>
          ...
          <div className='mt-1 min-h-5 text-sm text-red-600'>{errors.price_min?.message}</div>
        ...
        <li className='py-1 pl-2'>
          <Link to='' className='flex items-center text-sm'>
            {Array(5)
              .fill(0)
              .map((_, index) => (
                <svg viewBox='0 0 9.5 8' className='mr-2 h-4 w-4' key={index}>
                  <defs>
      ...
      <div className='my-4 h-[1px] bg-gray-300'></div>
      <Button className='w-full bg-orange p-2 text-sm uppercase text-white hover:bg-orange/80'>Xóa tất cả</Button>
    </div>
  )
}
